...
#import
vst_expr <- read_tsv("./variance_stabilized_counts_2025_01_15.tsv")
...
annotation_file <- ("Ppyr_annotations_ORmodified_2025_01_15.tsv")
...
#nrow(my.pheromone.data.df.expanded) #13,337
...
#export both tables (in nice order) as Supp table S4 (big table)
#write.table(my.DE.data.df.expanded, "2025_01_16_DE_table_expanded.txt", row.names = FALSE, sep ='\t')
#write.table(my.DE.OR.data.df.expanded, "2025_01_16_DE_OR_table_expanded.txt", row.names = FALSE, sep ='\t')
```
...
```
  ...
  + `r nrow(m_tissue_DE) #3692` genes are significantly differentially expressed between male antenna and legs
...
```
  ...
  + `r nrow(f_tissue_DE) #3018` genes are significantly differentially expressed between female antenna and legs
...
#ggsave("2025_01_15_volcanoes.svg", plot = volcanos.plot, device = "svg", width = 300, height = 300, dpi = 300, units = "mm")
```
...
#ggsave("2025_01_21_boxplots_free.svg", plot = boxplots.free, device = "svg", width = 170, height = 220, dpi = 300, units = "mm")
...
my.LG.data <- read.table("/Users/sel025/Library/CloudStorage/GoogleDrive-sel025@bucknell.edu/My Drive/2.Research/03_Signal_switching_grant_FUNDED/01.Manuscripts/02_Ppyr_OR_manuscript/For_GitHub/Photinus_pyralis_odorant_receptor_expression/bigger_OR_tibble_2025_1_15.txt", sep = "\t", header = TRUE)
...
g <- ggplot(LG_tibble, aes(x = LG_common_name, y = LG_length)) +
  geom_bar(stat = "identity", color = "black", fill = "white") +
  theme_classic() +
  coord_flip() +
  scale_x_discrete(limits = rev) +
...
#ggsave("Mant_v_mleg_expression_no_legend_on_LG_2025_01_21.svg", device = "svg")
#1 is ok, 7, 18, 20
  
h <- ggplot(LG_tibble, aes(x = LG_common_name, y = LG_length)) +
  geom_bar(stat = "identity", color = "black", fill = "white") +
  theme_classic() +
  coord_flip() +
  scale_x_discrete(limits = rev) +
...
#ggsave("Fant_v_fleg_expression_on_LG_2025_01_21.svg", device = "svg")

i <- ggplot(LG_tibble, aes(x = LG_common_name, y = LG_length)) +
  geom_bar(stat = "identity", color = "black", fill = "white") +
  theme_classic() +
  coord_flip() +
  scale_x_discrete(limits = rev) +
...
#ggsave("Mant_v_fant_expression_on_LG_2025_01_21.svg", device = "svg")

j <- ggplot(LG_tibble, aes(x = LG_common_name, y = LG_length)) +
  geom_bar(stat = "identity", color = "black", fill = "white") +
  theme_classic() +
  coord_flip() +
  scale_x_discrete(limits = rev) +
...
#ggsave("Mleg_v_fleg_expression_on_LG_2025_01_21.svg", device = "svg")
#for some reasno, jittering changes when you assign the plot to a variable (like the set.seed doesn't work - so saved individually and combined in adobe)
```
...
#ggsave("Male_ant_norm_counts_on_LG_2025_01_22.svg", device = "svg")
#1 is ok, 7, 18, 20
...
#ggsave("Female_ant_norm_counts_on_LG_2025_01_22.svg", device = "svg")
...
#ggsave("Male_leg_norm_counts_on_LG_2025_01_22.svg", device = "svg")
...
#ggsave("Female_leg_norm_counts_on_LG_2025_01_22.svg", device = "svg")
...
#ggsave("Expression_on_phylo_2025_01_22.svg", device = "svg")
```
